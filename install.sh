...
setup_env() {
    ...
    if [[ ! -f .env ]]; then
        ...
        sed -i "s/VPN_SERVER_IP=your-server-ip/VPN_SERVER_IP=${SERVER_IP}/" .env
...
start_panel() {
    ...
    PW_FILE="${INSTALL_DIR}/vpn-panel-password"
    if [[ ! -f "$PW_FILE" ]]; then
...
show_info() {
    PW_FILE="${INSTALL_DIR}/vpn-panel-password"
    if [[ -f "$PW_FILE" ]]; then
        PANEL_PASSWORD=$(cat "$PW_FILE")
    else
        PANEL_PASSWORD="N/A (check: cat ${PW_FILE})"
    fi
